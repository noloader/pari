...
of multivariate polynomial rings. Polynomials over non-commutative rings
are not supported.
...
non-commutative rings are not supported.
...
such as \kbd{VEC}, but you can retrieve entry values and assign to them as
...
%6 = [3, 2, 3]
@eprog\noindent
A \typ{VECMALL} entry is any signed integer $x$ such $|x| < 2^{31}$
(resp.~$|x| < 2^{63}$) on a 32-bit (resp.~64-bit) architecture. Assigning a
larger integer to a \typ{VECSMALL} triggers an exception:
\bprog
? v[1] = 2^63
