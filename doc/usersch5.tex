...
$v$ of all $e^i$, $0 \leq i \leq n$, where $v[i] = e^{i-1}$. In contrast to
\kbd{powruvec}, this function uses the \tet{gen_powers} convention, which
allows to use \tet{gen_bkeval}.
...
\subsec{Continued fractions and convegents}
...
\fun{GEN}{FpX_digits}{GEN x, GEN T, GEN p} returns a vector of \kbd{FpX}
$[c_0,\ldots,c_n]$ of degree less than the degree of $T$ and such that
$x=\sum_{i=0}^{n}{c_i\*T^i}$.

\fun{GEN}{FpX_fromdigits}{GEN v, GEN T, GEN p} where $v=[c_0,\ldots,c_n]$
is a vector of \kbd{FpX}, returns $\sum_{i=0}^{n}{c_i\*T^i}$.
