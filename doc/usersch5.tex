...
\fun{GEN}{Flxn_inv}{GEN x, long e, ulong p}
...
\typ{INT} $N$ using $n$ rounds of Pollard iterations; \var{seed} is an integer
whose value (mod $8$) selects the quadratic polynomial use to generate
Pollard's (pseudo)random walk. Returns \kbd{NULL} on failure, else a vector
of 2 (possibly 3) integers whose product is $N$.
...
\fun{GEN}{ZM_pseudoinv}{GEN M, GEN *pd} if \kbd{M} is a \kbd{ZM} with full
column rank, return a primitive left-inverse $H$ such that $H M$ is $d$ times
the identity and set \kbd{*pd} to $d$.
...
elements of a small group, a \typ{VECSMALL} \kbd{conj} of the same length
