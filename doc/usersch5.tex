...
permutations (as \typ{VECSMALL} or \typ{VEC} of integers) listing the group
elements. Returns the list of group elements as permutations
(\typ{VECSMALL}).
...
\fun{GEN}{groupelts_conjclasses}{GEN elts, long *pt_n}, where \var{elts} is
...
\kbd{pt\_n} is non NULL, \kbd{*pt\_n} is set to $n$.

\fun{GEN}{conjclasses_repr}{GEN conj, long nb},
where \kbd{conj} and \kbd{nb} as returned by \kbd{groupelts\_conjclasses(elts)},
return \typ{VECSMALL} of length \kbd{nb} which gives the indices in \kbd{elts}
of a representative of each conjugacy class.

\fun{GEN}{groupelts_to_conjclasses}{GEN elts}, where \var{elts} is the list
of elements of a small group, return \kbd{[elts,conj,rep]}, where
\kbd{conj} is \kbd{groupelts\_conjclasses}$(\kbd{elts})$ and
\kbd{rep} is the attached \kbd{conjclasses\_repr}.
