...
\fun{ulong}{nonsquare_Fl}{ulong p} return a quadratic non-residue modulo $p$,
assuming $p$ is an odd prime. If $p$ is $3$ mod $4$, return $p-1$, else
return the smallest (prime) non-residue.
...
\fun{GEN}{ZV_lcm}{GEN A} given a vector of $n$ integers $A$,
...
is non-zero, normalize \kbd{*T} as an \kbd{FpX}; on the other hand,
...
\fun{GEN}{Z_factor_limit_strict}{GEN n, ulong all, GEN *pU} as
\tet{Z_factor_limit}, returning the factorization matrix of the smooth
part of $n$, without the last possibly composite divisor.
If \kbd{pU} is not \kbd{NULL}, set it to the cofactor, which is either
\kbd{NULL} (no cofactor) or a divisor of $n$ whose prime divisors are greater
that \kbd{all}.
...
entries are the non-zero coefficients ($1$).
...
non-rational root in the open interval \kbd{(a,b)} or elements \kbd{x0} such
...
as \kbd{RgX\_deflate\_order} where $F$ is a non-constant \typ{RFRAC}.

\fun{long}{rfrac_deflate_max}{GEN F, long *d}
as \kbd{RgX\_deflate\_max} where $F$ is a non-constant \typ{RFRAC}.
...
\fun{GEN}{RgV_equiv}{GEN O} given a vector of objects $O$, return a vector
...
\fun{GEN}{RgV_count}{GEN *pO} given a vector of objects $O = \kbd{*pO}$,
return a \typ{VECSMALL} $E$ with $n$ components where $n$ is the number of
distinct objects in $O$ and $E[i]$ is the multiplicity of object $O[i]$.
Replace \kbd{*O} by the set of distinct objects. Shallow function.
