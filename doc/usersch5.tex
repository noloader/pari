...
defaults that make sense in libpari context, and \tet{functions_gp_default}
containing all \kbd{gp}-specific defaults.
...
\fun{char*}{gp_filter}{const char *s, int flag} pre-processor for the GP
parser: filter out whitespace and GP comments from $s$. If \fl is non-zero,
downcase all identifiers (letters outside of \kbd{"..."} GP strings).

\fun{GEN}{pari_compile_str}{const char *s, int strict} low-level form of
...
that GP metacommands are not recognized. The \kbd{strict} parameter is likely
to disappear in the future; if $0$ only warn if unused characters are left at
the end of the expression, see \kbd{strictmatch}; otherwise, raise an
exception.
...
\fun{GEN}{FpX_powu}{GEN x, ulong n, ulong p} returns $x^n$.
...
$P$ being a vector of integer (or \typ{VECSMALL}s) of length $n$,
