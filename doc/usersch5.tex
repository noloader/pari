...
primes. This functionis called by \kbd{pari\_init(\dots,maxprime)}.
It needs to be called manuall when using the flag \kbd{INIT\_noPRIMEm}
to \kbd{pari\_init\_opts}.
...
Set the default \kbd{parisize} to \kbd{rsize} and the
...
Change the current stack size to \kbd{newsize}
...
Reset the current stack to its default size \kbd{parisize},
...
\fun{GEN}{veczeta}{GEN a, GEN b, long N, long prec} returns the vector
