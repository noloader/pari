...
\fun{ulong}{Flx_Flv_eval}{GEN P, GEN v, ulong y, ulong p} returns the vector
...
the relative precision in bits. If \kbd{flag} is 0 returns a list of intervals
that isolate the real roots of \kbd{P}. In that case the return value is a
column of elements which are either vectors \kbd{[a,b]} meaning that there is a
single root in the open interval \kbd{(a,b)} or elements \kbd{x0} such that
\kbd{x0} is a root of \kbd{P}. There is no guarantee that all rational roots
will be found (at most those with power of $2$ denominator can be found and
even those are not guaranteed to be found). Beware that the limits of the open
intervals can be roots of the polynomial. If \kbd{flag} is 1 returns an
approximation of the real roots of \kbd{P}. The argument \kbd{ab} specify the
interval in which the roots are searched. The default interval is
$(-\infty,\infty)$. If \kbd{ab} is an integer or fraction $a$ then the interval
will be $[a,\infty)$. If \kbd{ab} is a vector of two integers or fractions $a$
e $b$, the the interval will be $[a,b]$.
