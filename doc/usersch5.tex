...
Return $x^n$, where $R$ is as given by \kbd{Fp\_pow\_init(x,m,k,p)}
...
\fun{GEN}{divisorsu_P}{GEN P} returns the divisors of $n$ of the form
$\prod_{p\in S} p$, $S\subset P$. In a \typ{VECSMALL}, sorted by increasing order.
If $P$ is \kbd{factoru(n)[1]}, this returns the squarefree divisors of $n$.
...
\fun{void}{ZC_lincomb1_inplace_i}{GEN X, GEN Y, GEN v, long n} internal
version of \tet{ZC_lincomb1_inplace}: only update $X[1], \dots, X[n]$,
