...
\kbd{pt} to evaluate \kbd{worker} in parallel. This allocates data in
...
of \kbd{tasks} to perform. Concretly the number of threads is the minimum
...
needs to be submitted. The label \kbd{taskid} is attached to a task but is
not available to \kbd{worker} and its main use is for the main thread to
later match up results and submitted tasks (see \kbd{mt\_queue\_get} which
returns the \kbd{taskid} together with each result). In practice using
consecutive (distinct!) integers should be sufficient. The \kbd{taskid}
parameter is ignored when \kbd{task} is \kbd{NULL}. It is forbidden to call
this function twice without an intervening \kbd{mt\_queue\_get}.
...
submitted tasks, in random order. Set \kbd{pending} to the number of
remaining pending tasks: if this is $0$ then no more tasks are pending and it
is safe to call \tet{mt_queue_end}. Set \kbd{*taskid} to the value attached
to this task by \kbd{mt\_queue\_submit}, unless the \kbd{taskid} pointer is
\kbd{NULL}. Returns \kbd{NULL} if all tasks submitted so far have been
...
@eprog \noindent instead of our use of a temporary \kbd{done} would be
