...
$\epsilon$
of modulus $1$, and an associated sequence $a^*$ defining both an
$L$-function $L(a^*,s)$ satisfying the above two assumptions and a completed
function $\Lambda(a^*,s) = N^{s/2}\gamma_A(s) \cdot L(a^*,s)$, such that
$$\Lambda(a,k-s) = \epsilon \Lambda(a^*,s)$$
for all regular points.

More often than not in number theory we have $a^* = \overline{a}$, but this
needs not be the case. If $a$ is a real sequence and $a = a^*$, we say that
$L$ is \emph{self-dual}. We do not assume that the $a_n$ are multiplicative,
nor equivalently that $L(s)$ has an Euler product.
...
One can always increase \kbd{realprecision} and recompute: the difference
...
\item an \tet{Lmath} is an arbitray description of the underlying
...
$D$, the function $L((D/.), s)$ is encoded by the \typ{INT} $D$. This
includes Riemann $\zeta$ function via the special case $D = 1$.

Non-quadratic (primitive) characters of small conductor $N$ can be represented
by a vector with two components $[n,v]$, where $n$ is the order of the
character, $N = \kbd{\#}v$ and $\chi(j) = \exp(2\pi i \cdot v[j]/n)$
for all $1 \leq j \leq N$, $(j,N) = 1$.

For general characters of large conductor, use Hecke characters over $K = \Q$
(see below).

\subsec{Hecke $L$-functions}

Given a finite order Hecke character $\chi: Cl_f(K)\to \C$, we let
$$L(\chi, s) = \sum_{A \subset O_K} \chi(A)\, \left(N_{K/\Q}A\right)^{-s}.$$
Only primitive characters are supported. If $T$ is an irreducible rational
polynomial or any absolute number field structure (preferably at least
a \var{bnf}), then $T$ is associated to the Dedekind zeta function (trivial
$f$ and $\chi$).
...
a_i n_i / d_i)$. The pair $[\var{bnr}, v]$ encodes $\chi$.
...
@eprog\noindent
For even characters, replace by \kbd{bnrinit(K, N)}. Note that if the
character is quadratic or $N$ is small, the simpler direct construction in the
previous section will be more efficient.

\subsec{Hasse-Weil $L$-functions}
...
\subsec{Eta quotients / Modular forms}
