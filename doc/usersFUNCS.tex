...
More general characters can be represented in a variety of way:

\item via Conrey notation (see \tet{znconreychar}): $\chi_N(m,\cdot)$
can be given as the \typ{INTMOD} \kbd{Mod(m,N)}.
...
\var{bid} structure $G$ (\kbd{G.cyc} give the $d_i$ and \kbd{G.gen} the
