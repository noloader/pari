...
$e_2 > 1$ and additionally $3$ if $e_2 > 2$). We can again write $G =
...
$2$ and $3$ has order $2^{e_2-2}$ $(e_2 > 2)$. Nevertheless, any $m\in
...
\item The \emph{column vector} of the $m_j$ is called the \tev{Conrey
logarithm} of $m$ (discrete logarithm in terms of the Conrey  generators).
Note that discrete logarithms in PARI/GP are always expressed as \typ{COL}s.
...
generators) or a \typ{VEC} (in  terms of the SNF generators)
...
\kbd{charker}, \kbd{chareval}, \kbd{charorder}, \kbd{znconreyconductor}
(also computes the primitive character attached to the input character).
