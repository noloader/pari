...
\li{Galois orbits of Dirichlet characters}{znchargalois$(G)$}
...
\li{initialize $M_k(\Gamma_0(N),\chi)$}{mfinit$([N,k,\chi],3)$}
\li{initialize $E_k(\Gamma_0(N),\chi)$}{mfinit$([N,k,\chi],4)$}
\li{split $S_k^{\text{new}}$ into eigenspaces}{mfsplit$(M)$}
\li{statistics on self-growing caches}{getcache$()$}
\smallskip

We let $M$ = \kbd{mfinit}$(\dots)$ denote a modular space and let $T$ =
\kbd{mfsplit}$(M)$ be its splitting. All functions accepting $M$ also
allow $T$.
\hfil\break
...
\li{\dots a basis $(F_j)$ of eigenforms}{mfeigenbasis$(T)$}
\li{\dots polynomials defining $\QQ(\chi)(F_j)/\QQ(\chi)$}{mffields$(T)$}
\smallskip
\li{matrix of Hecke operator $T_n$ on $(f_i)$}{mfmathecke$(M,n)$}
\li{\dots of Atkin--Lehner $w_Q$ on $(f_i)$}{mfmatatkin$(T,Q)$}
\li{eigenvalues of $w_Q$}{mfatkineigenvalues$(T,Q)$}
\li{period polynomial of eigenform $F$}{mfperiodpol$(F)$}
...
\kbd{mfeigenbasis}, an individual modular form can be created by a recipe to
compute its coefficients.\hfil\break
\li{modular form from coefficients}{mfcreate($n$->$a(n)$)}
\smallskip

There are also many predefined ones:\hfil\break
\li{Eisenstein series $E_k$ on $Sl_2(\ZZ)$}{mfEk$(k)$}
\li{Ramanujan's $\Delta$}{mfDelta$()$}
\li{$E_k(\chi)$}{mfeisen$(k,\chi)$}
\li{$E_k(\chi_1,\chi_2)$}{mfeisen$(k,\chi_1,\chi_2)$}
\li{eta quotient $\prod_i \eta(a_{i,1} \cdot z)^{a_{i,2}}$}{mfetaquo$(a)$}
...
\li{Hecke operator $T_n f$}{mfhecke$(f,n)$}
\li{Atkin--Lehner operator $w_Q f$}{mfatkin$(T,f,Q)$}
\li{twist by the quadratic char $(D/\cdot)$}{mftwist$(f,D)$}
\li{derivative wrt. $q \cdot d/dq$}{mfderiv$(f)$}
\li{integration wrt. $q \cdot d/dq$}{mfinteg$(f)$}
\li{Serre derivative $(q \cdot d/dq)f - k E_2 f/12$}{mfderivE2$(f)$}
\li{Rankin-Cohen bracket $[f,g]_n$}{mfmulRC$(f,g,n)$}
\li{complex embeddings of $f$}{mfembed$(f)$}

\subsec{Properties of modular forms} In this section, $f = \sum_n f_n q^n$
is a modular form
in some space $M$ with parameters $N,k,\chi$; an argument $T$ is
\kbd{mfsplit}$(M)$\hfil\break
\li{$(N,k,\chi)$ for form $f$}{mfparams$(f)$}
\li{the space identifіer (0 to 4) for $f$}{mfspace$(f)$}
\li{$f_0,\dots,f_n$}{mfcoefs$(f, n)$}
\li{$f_n$}{mfcoef$(f,n)$}
\li{$v_\infty(f)$}{mfval$(f)$}
\li{is $f$ cuspidal?}{mfiscuspidal$(f)$}
\li{is $f$ an eigenform of Fricke's $w_N$?}{mfisselfdual$(f)$}
...
\li{decompose $f$ on $\oplus S_k^{\text{new}}(\Gamma_0(d))$, $d\mid N$}
   {mftonew$(M,f)$}
\li{Fourier expansion of $f$ at cusp $c$}{mfcuspexpansion$(T, f, c, n)$}
\li{$n$-Taylor expansion of $f$ at $i$}{mftaylor$(f,n)$}
\li{all rational eigenforms matching criteria}{mfsearch}
...
\li{$L$-function attached to $F$}{mflfuncreate$(F)$}
\li{\dots eigenforms of split new space $T$}{mflfuncreate$(T)$}
