...
for $e$ (in \kd{ellinit} form, over $\Q$). Set $v = \kbd{NULL}$ (already
...
as above for $E$, except that \tet{t_ELL_Fq} are not allowed.
In addition the $j$ invariant of $E$ must not be equal to $0$ or $1728$
modulo $p$.
...
a prime are acceptable, set $s=2$. There is no guarantee that the resulting
...
the Frobenius of $E(\F_p)$. The cardinal of $E(\F_p)$ is thus $p+1-t$, which
might not fit in an \kbd{ulong}.
