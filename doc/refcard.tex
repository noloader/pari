...
\li{\typ{FFELT}. Elt in finite field $\F_q$}{ffgen($q$,\text{'t})}
...
\li{Create $t$ in $\F_q\simeq\F_p[t]/(T)$}{t = ffgen$(T,\text{'t})$}
\li{\dots indirectly, with implicit $T$}{t = ffgen$(q,\text{'t})$; T = t.mod}
\li{map $m$ from $\F_q\ni a$ to $\F_{q^k} \ni b$}{m = ffembed$(a,b)$}
\li{build $K=\F_q[x]/(P)$ extending $\F_q\ni a$, }{ffextend$(a, P)$}
...
\li{ECPP certificate for (prime) $N$}{ecpp$(N)$}
\li{verifies an ECPP certificate $c$}{ecppisvalid$(c)$}
\li{convert certificate to Magma/PRIMO}{ecppexport}
